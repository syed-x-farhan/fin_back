...
from services.historical import calculate_historical_statements, HistoricalServiceFactory
from services.historical.financial_statements_service import FinancialStatementsService
from services.historical.dashboard import DashboardServiceFactory
from services.dcf_calculation import (
    ...
    calculate_sensitivity_analysis, calculate_risk_analysis
)
...
# Historical Model Routes (must come before general routes to avoid conflicts)

@models_router.post("/historical/calculate")
async def calculate_historical_model(request: Dict[str, Any] = Body(...)) -> ApiResponse:
    """Calculate historical financial model based on input data"""
    try:
        print("=== API ROUTE DEBUG ===")
        print(f"Received request keys: {list(request.keys())}")
        print(f"Request data: {request}")
        
        # Get company type from request
        company_type = request.get('companyType', 'service')
        print(f"Company type: {company_type}")
        
        # Validate company type
        if not HistoricalServiceFactory.is_company_type_supported(company_type):
            available_types = list(HistoricalServiceFactory.get_available_company_types().keys())
            raise HTTPException(
                status_code=400, 
                detail=f"Company type '{company_type}' not supported. Available types: {available_types}"
            )
        
        print("Company type validated, proceeding with calculation...")
        
        # Perform historical calculation using factory (for financial statements)
        statements_result = HistoricalServiceFactory.calculate_historical_statements(company_type, request)
        
        print(f"Statements calculation completed, result keys: {list(statements_result.keys()) if statements_result else 'None'}")
        
        if not statements_result.get('success', False):
            raise HTTPException(
                status_code=400,
                detail=f"Statements calculation failed: {statements_result.get('errors', 'Unknown error')}"
            )
        
        # Calculate dashboard metrics using the statements data
        statements_data = {
            'income_statement': statements_result.get('income_statement', {}),
            'balance_sheet': statements_result.get('balance_sheet', {}),
            'cash_flow': statements_result.get('cash_flow', {}),
            'original_data': request  # Pass original request data for service-specific metrics
        }
        
        dashboard_result = DashboardServiceFactory.calculate_dashboard_metrics(company_type, statements_data)
        
        print(f"Dashboard calculation completed, success: {dashboard_result.get('success', False)}")
        
        # Combine results - statements + dashboard KPIs
        combined_result = statements_result.copy()
        
        if dashboard_result.get('success', False):
            dashboard_data = dashboard_result.get('data', {})
            # Add dashboard KPIs to the main result
            combined_result['dashboard_kpis'] = dashboard_data.get('dashboard_kpis', {})
            combined_result['dashboard_data'] = dashboard_data  # Full dashboard data for advanced features
        else:
            print(f"Dashboard calculation failed: {dashboard_result.get('error', 'Unknown error')}")
            # Continue without dashboard data - statements are still valid
            combined_result['dashboard_kpis'] = {}
            combined_result['dashboard_data'] = {}
...
@models_router.post("/historical/financial-statements")
async def process_financial_statements(request: Dict[str, Any] = Body(...)) -> ApiResponse:
    """Process imported financial statements and generate forecasts"""
    try:
        print("=== FINANCIAL STATEMENTS API ROUTE DEBUG ===")
        print(f"Received request keys: {list(request.keys())}")
        
        # Debug the original data structure
        original_data = request.get('_originalData', {})
        print(f"Original data keys: {list(original_data.keys())}")
        if 'incomeStatement' in original_data:
            income_data = original_data['incomeStatement']
            print(f"Income statement keys: {list(income_data.keys())}")
            print(f"Income statement years: {income_data.get('years', 'MISSING')}")
            print(f"Income statement lineItems count: {len(income_data.get('lineItems', []))}")
        
        # Initialize financial statements service
        service = FinancialStatementsService()
        
        # Process the financial statements
        result = service.process_financial_statements(request)
        
        print(f"Processing completed, result success: {result.get('success', False)}")
        
        if result.get('success', False):
            return ApiResponse(
                success=True,
                message="Financial statements processed successfully",
                data=result
            )
        else:
            ...
            return ApiResponse(
                success=False,
                message="Financial statements processing failed",
                data=result,
                error=error_message
            )
        
    except Exception as e:
        print(f"=== FINANCIAL STATEMENTS API ERROR ===")
        print(f"Error: {str(e)}")
        import traceback
        print(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))
...
@models_router.get("/historical/company-types")
async def get_historical_company_types() -> ApiResponse:
    """Get available company types for historical calculations"""
    try:
        company_types = HistoricalServiceFactory.get_available_company_types()
...
@models_router.get("/historical/company-types/{company_type}")
async def get_historical_company_type_info(company_type: str) -> ApiResponse:
    """Get information about a specific company type"""
    try:
        service_info = HistoricalServiceFactory.get_service_info(company_type)
        
        if not service_info:
            available_types = list(HistoricalServiceFactory.get_available_company_types().keys())
            raise HTTPException(
                status_code=404,
                detail=f"Company type '{company_type}' not found. Available types: {available_types}"
            )
        
        return ApiResponse(
            success=True,
            message=f"Company type information retrieved successfully",
            data=service_info
        )
...
@models_router.post("/historical/validate")
async def validate_historical_data(request: Dict[str, Any] = Body(...)) -> ApiResponse:
    ...
    try:
        ...
        # Validate data
        validation_result = HistoricalServiceFactory.validate_historical_data(company_type, data)
...
@models_router.post("/historical/dashboard-metrics")
async def calculate_historical_dashboard(request: Dict[str, Any] = Body(...)) -> ApiResponse:
    """Calculate dashboard metrics for historical data"""
    try:
        print("=== DASHBOARD METRICS API ROUTE DEBUG ===")
        print(f"Received request keys: {list(request.keys())}")
        ...
        # Validate company type
        if not DashboardServiceFactory.is_company_type_supported(company_type):
            available_types = list(DashboardServiceFactory.get_available_company_types().keys())
            raise HTTPException(
                status_code=400, 
                detail=f"Company type '{company_type}' not supported. Available types: {available_types}"
            )
        
        # Get statements data from request
        statements_data = request.get('statementsData', {})
        if not statements_data:
            raise HTTPException(
                status_code=400,
                detail="Statements data is required for dashboard calculation"
            )
        
        print("Company type validated, proceeding with dashboard calculation...")
        
        # Calculate dashboard metrics using factory
        result = DashboardServiceFactory.calculate_dashboard_metrics(company_type, statements_data)
        
        print(f"Dashboard calculation completed, result success: {result.get('success', False)}")
        
        if not result.get('success', False):
            raise HTTPException(
                status_code=400,
                detail=f"Dashboard calculation failed: {result.get('error', 'Unknown error')}"
            )
        ...
        print(f"=== DASHBOARD API ROUTE ERROR ===")
        print(f"Error: {str(e)}")
        import traceback
        print(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))
...
@models_router.get("/historical/dashboard/company-types")
async def get_dashboard_company_types() -> ApiResponse:
    """Get available company types for dashboard calculations"""
    try:
        company_types = DashboardServiceFactory.get_available_company_types()
        
        return ApiResponse(
            success=True,
            message="Dashboard company types retrieved successfully",
            data=company_types
        )
...
@models_router.get("/historical/dashboard/company-types/{company_type}")
async def get_dashboard_company_type_info(company_type: str) -> ApiResponse:
    """Get information about a specific dashboard company type"""
    try:
        service_info = DashboardServiceFactory.get_service_info(company_type)
        
        if not service_info:
            available_types = list(DashboardServiceFactory.get_available_company_types().keys())
            raise HTTPException(
                status_code=404,
                detail=f"Dashboard company type '{company_type}' not found. Available types: {available_types}"
            )
        
        return ApiResponse(
            success=True,
            message=f"Dashboard company type '{company_type}' information retrieved successfully",
            data=service_info
        )
...
@models_router.get("/historical/dashboard/{company_type}/chart-types")
async def get_dashboard_chart_types(company_type: str) -> ApiResponse:
    """Get available chart types for a specific company type"""
    try:
        chart_types = DashboardServiceFactory.get_dashboard_chart_types(company_type)
        ...
        return ApiResponse(
            success=True,
            message=f"Chart types for '{company_type}' retrieved successfully",
            data=chart_types
        )
...
@models_router.get("/historical/dashboard/{company_type}/kpi-categories")
async def get_dashboard_kpi_categories(company_type: str) -> ApiResponse:
    """Get KPI categories for a specific company type"""
    try:
        kpi_categories = DashboardServiceFactory.get_kpi_categories(company_type)
        
        if kpi_categories is None:
            available_types = list(DashboardServiceFactory.get_available_company_types().keys())
            raise HTTPException(
                status_code=404,
                detail=f"Company type '{company_type}' not found. Available types: {available_types}"
        ...
        return ApiResponse(
            success=True,
            message=f"KPI categories for '{company_type}' retrieved successfully",
            data=kpi_categories
        )
...
async def calculate_model(model_id: str, request: Dict[str, Any] = Body(...)) -> ApiResponse:
    ...
    try:
        ...
        print(f"[DEBUG] Forecast data structure: {forecast[0] if forecast else 'No forecast data'}")
        if forecast:
            print(f"[DEBUG] First year forecast - Revenue: {forecast[0].get('revenue', 0)}, COGS: {forecast[0].get('cogs', 0)}, Gross Profit: {forecast[0].get('gross_profit', 0)}")
        free_cash_flows = [year.get("free_cash_flow", 0) for year in forecast]
        
        # For investment analysis, we need to include initial investment
        # Calculate initial investment from the data
        initial_investment = 0.0
        if data.get("selfFunding"):
            initial_investment += float(data.get("selfFunding", 0))
        if data.get("equipment"):
            for eq in data.get("equipment", []):
                initial_investment += float(eq.get("cost", 0))
        if data.get("investments"):
            for inv in data.get("investments", []):
                if not inv.get("income", False):  # Only count non-income investments
                    initial_investment += float(inv.get("amount", 0))
        
        # Create investment cash flows: initial investment (negative) + free cash flows
        investment_cash_flows = [-initial_investment] + free_cash_flows
        
        print(f"[DEBUG] Initial investment: {initial_investment}")
        print(f"[DEBUG] Original free cash flows: {free_cash_flows}")
        print(f"[DEBUG] Investment cash flows (with initial investment): {investment_cash_flows}")
        ...
        # Use investment cash flows for NPV calculation
        npv = calculate_npv(investment_cash_flows, discount_rate)
        
        # Debug logging for cash flows
        print(f"[DEBUG] Free cash flows: {free_cash_flows}")
        print(f"[DEBUG] Cash flows length: {len(free_cash_flows)}")
        print(f"[DEBUG] First cash flow: {free_cash_flows[0] if free_cash_flows else 'None'}")
        print(f"[DEBUG] All cash flows positive: {all(cf >= 0 for cf in free_cash_flows) if free_cash_flows else 'No cash flows'}")
        print(f"[DEBUG] All cash flows negative: {all(cf <= 0 for cf in free_cash_flows) if free_cash_flows else 'No cash flows'}")
        
        try:
            # Use investment cash flows for IRR calculation
            irr = calculate_irr(investment_cash_flows)
            print(f"[DEBUG] IRR calculation result: {irr}")
        except Exception as e:
            print(f"[DEBUG] IRR calculation failed: {e}")
            irr = None
        try:
            # Use investment cash flows for payback period calculation
            payback_period = calculate_payback_period(investment_cash_flows)
            print(f"[DEBUG] Payback period calculation result: {payback_period}")
        except Exception as e:
            print(f"[DEBUG] Payback period calculation failed: {e}")
            payback_period = None
        ...
        print(f"[DEBUG] Final valuation results:")
        print(f"[DEBUG] - IRR: {irr}")
        print(f"[DEBUG] - NPV: {npv}")
        print(f"[DEBUG] - Payback Period: {payback_period}")
        print(f"[DEBUG] - Enterprise Value: {enterprise_value}")
        print(f"[DEBUG] - Equity Value: {equity_value}")
        
        result["dcf"] = {"dcf_value": dcf_value, "discount_rate": discount_rate, "terminal_value": terminal_value, "terminal_growth": terminal_growth, "free_cash_flows": free_cash_flows, "investment_cash_flows": investment_cash_flows, "initial_investment": initial_investment, "tv_method": tv_method, "tv_metric": tv_metric, "tv_multiple": tv_multiple, "tv_custom_value": tv_custom_value, "tv_year": tv_year}
...
@models_router.post("/risk-analysis")
async def risk_analysis_endpoint(
    request: Dict[str, Any] = Body(...)
):
    """
    Calculate risk analysis probabilities. Expects:
    {
      "free_cash_flows": [number, ...],
      "base_discount_rate": 0.1,
      "base_terminal_growth": 0.02,
    ...
    try:
        ...
        if not free_cash_flows:
            return JSONResponse({"error": "Free cash flows are required"}, status_code=400)
        
        # Calculate risk analysis
        risk_results = calculate_risk_analysis(
            free_cash_flows, base_discount_rate, base_terminal_growth, runs
        )
        
        return JSONResponse(risk_results)
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500) 