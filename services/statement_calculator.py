...
def calculate_loan_payments(loan: Dict[str, Any], forecast_years: int, base_year: int, 
                          interest_rates: Dict[str, float], data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Calculate loan payments based on loan type with professional financial modeling.
    
    Args:
        loan: Loan data dictionary
        forecast_years: Number of forecast years
        base_year: Base year for calculations
        interest_rates: Global interest rates
        data: Full input data for context
    
    Returns:
        Dictionary with loan calculations including amortization table, cash flows, and balance sheet impacts
    """
    loan_type = loan.get('loanType', 'working_capital')
    loan_amount = float(loan.get('amount', 0))
    annual_rate = float(loan.get('rate', 6)) / 100
    years_loan = int(loan.get('years', forecast_years))
    
    # Use global rates if enabled
    if interest_rates.get('use_for_loans', False):
        if years_loan <= 1:
            annual_rate = interest_rates['short_term']
        else:
            annual_rate = interest_rates['long_term']
    
    # Determine loan start year
    ...
    result = {
        ...
        'years_loan': years_loan,
        'start_idx': loan_start_idx,
        'amortization_table': [],
        'interest_by_year': [0.0] * forecast_years,
        'principal_by_year': [0.0] * forecast_years,
        'cash_flow_impact': [0.0] * forecast_years,
        'balance_sheet_impact': {
            'short_term_debt': [0.0] * forecast_years,
            'long_term_debt': [0.0] * forecast_years,
            'principal_remaining': [0.0] * forecast_years
        }
    }
    
    # Calculate based on loan type
    if loan_type == 'working_capital':
        result.update(calculate_working_capital_loan(loan, result, forecast_years))
    elif loan_type == 'sme_loan':
        result.update(calculate_sme_loan(loan, result, forecast_years))
    elif loan_type == 'trade_finance':
        result.update(calculate_trade_finance_loan(loan, result, forecast_years))
    elif loan_type == 'letter_of_guarantee':
        result.update(calculate_letter_of_guarantee(loan, result, forecast_years))
    elif loan_type == 'startup_loan':
        result.update(calculate_startup_loan(loan, result, forecast_years))
    else:
        # Default to standard loan calculation
        result.update(calculate_standard_loan(loan, result, forecast_years))
    
    return result

def calculate_working_capital_loan(loan: Dict[str, Any], base_result: Dict[str, Any], forecast_years: int) -> Dict[str, Any]:
    """
    Calculate working capital loan with revolving credit facility features.
    """
    loan_amount = base_result['loan_amount']
    annual_rate = base_result['annual_rate']
    revolving_limit = float(loan.get('revolvingLimit', loan_amount))
    utilization_rate = float(loan.get('utilizationRate', 80)) / 100
    
    # Working capital loans are typically revolving with variable utilization
    effective_loan_amount = loan_amount * utilization_rate
    
    # Generate amortization table (working capital loans may have bullet payments)
    amort_table = generate_amortization_table(effective_loan_amount, annual_rate, base_result['years_loan'], periods_per_year=1)
    
    # Calculate interest based on actual utilization
    interest_by_year = []
    principal_by_year = []
    ...
    for i in range(forecast_years):
        if i < len(amort_table):
            ...
            outstanding_balance = effective_loan_amount - sum(principal_by_year)
            annual_interest = outstanding_balance * annual_rate
            principal_payment = amort_table[i]['principal'] if i < len(amort_table) else 0
            
            interest_by_year.append(annual_interest)
            principal_by_year.append(principal_payment)
        else:
            interest_by_year.append(0.0)
            principal_by_year.append(0.0)
    
    return {
        'amortization_table': amort_table,
...
def calculate_sme_loan(loan: Dict[str, Any], base_result: Dict[str, Any], forecast_years: int) -> Dict[str, Any]:
    """
    Calculate SME loan with collateral considerations.
    """
    loan_amount = base_result['loan_amount']
    annual_rate = base_result['annual_rate']
    collateral_type = loan.get('collateralType', 'Unsecured')
    
    # SME loans typically have longer terms and may have grace periods
    # Add 1-2 year grace period for SME loans
    grace_period = 1
    effective_years = base_result['years_loan'] + grace_period
    
    # Generate amortization table with grace period
    amort_table = generate_amortization_table(loan_amount, annual_rate, effective_years, periods_per_year=1)
    
    # Adjust for grace period (no principal payments in first year)
    interest_by_year = []
    principal_by_year = []
    ...
    for i in range(forecast_years):
        if i < grace_period:
            ...
            interest_by_year.append(annual_interest)
            principal_by_year.append(0.0)
        elif i - grace_period < len(amort_table):
            # Regular payments after grace period
            table_idx = i - grace_period
            interest_by_year.append(amort_table[table_idx]['interest'])
            principal_by_year.append(amort_table[table_idx]['principal'])
        else:
            interest_by_year.append(0.0)
            principal_by_year.append(0.0)
...
def calculate_trade_finance_loan(loan: Dict[str, Any], base_result: Dict[str, Any], forecast_years: int) -> Dict[str, Any]:
    """
    Calculate trade finance loan (LC, Bills Discounting) with tenor considerations.
    """
    loan_amount = base_result['loan_amount']
    annual_rate = base_result['annual_rate']
    sub_type = loan.get('subType', 'letter_of_credit')
    tenor_days = float(loan.get('tenor', 90))
    trade_document_type = loan.get('tradeDocumentType', 'Invoice')
    
    # Trade finance loans are typically short-term
    # Convert tenor to years for calculation
    tenor_years = tenor_days / 365
    
    if sub_type == 'letter_of_credit':
        # LC fees are typically 1-2% of the amount
        lc_fee_rate = 0.015  # 1.5% typical LC fee
        lc_fee = loan_amount * lc_fee_rate
        
        # Interest is calculated on the tenor period
        interest_amount = loan_amount * annual_rate * tenor_years
        
        # LC payments are typically bullet payments
        interest_by_year = [0.0] * forecast_years
        principal_by_year = [0.0] * forecast_years
        
        # LC fee is paid upfront
        if base_result['start_idx'] < forecast_years:
            interest_by_year[base_result['start_idx']] = lc_fee
        
        # Principal and interest paid at maturity
        maturity_idx = min(base_result['start_idx'] + int(tenor_years), forecast_years - 1)
        if maturity_idx < forecast_years:
            principal_by_year[maturity_idx] = loan_amount
            interest_by_year[maturity_idx] += interest_amount
        
        return {
            'amortization_table': [],  # LCs don't have traditional amortization
            'interest_by_year': interest_by_year,
            'principal_by_year': principal_by_year,
            'lc_fee': lc_fee,
            'tenor_days': tenor_days,
            'trade_document_type': trade_document_type
        }
    
    elif sub_type == 'bills_discounting':
        # Bills discounting: discount the face value
        discount_rate = annual_rate
        discount_amount = loan_amount * discount_rate * tenor_years
        proceeds = loan_amount - discount_amount
        
        interest_by_year = [0.0] * forecast_years
        principal_by_year = [0.0] * forecast_years
        
        # Discount is effectively the interest cost
        if base_result['start_idx'] < forecast_years:
            interest_by_year[base_result['start_idx']] = discount_amount
...
def calculate_letter_of_guarantee(loan: Dict[str, Any], base_result: Dict[str, Any], forecast_years: int) -> Dict[str, Any]:
    """
    Calculate letter of guarantee with guarantee fees.
    """
    loan_amount = base_result['loan_amount']
    guarantee_amount = float(loan.get('guaranteeAmount', loan_amount))
    
    # Letters of guarantee typically charge 1-3% annual fee
    guarantee_fee_rate = 0.02  # 2% annual fee
    annual_guarantee_fee = guarantee_amount * guarantee_fee_rate
    
    interest_by_year = [0.0] * forecast_years
    principal_by_year = [0.0] * forecast_years
    
    # Guarantee fees are paid annually
    for i in range(forecast_years):
        if i >= base_result['start_idx']:
            interest_by_year[i] = annual_guarantee_fee
...
def calculate_startup_loan(loan: Dict[str, Any], base_result: Dict[str, Any], forecast_years: int) -> Dict[str, Any]:
    """
    Calculate startup loan with equity, royalty, or fixed payment structures.
    """
    loan_amount = base_result['loan_amount']
    sub_type = loan.get('subType', 'fixed')
    
    if sub_type == 'equity':
        equity_stake = float(loan.get('equityStake', 10)) / 100  # Convert percentage to decimal
        
        # Equity-based loans: no interest payments, but equity dilution
        interest_by_year = [0.0] * forecast_years
        principal_by_year = [0.0] * forecast_years
        
        # Equity is typically converted at a future date or exit
        # For modeling purposes, we'll treat it as a long-term liability
        for i in range(forecast_years):
            if i >= base_result['start_idx']:
                # No interest payments for equity-based loans
                interest_by_year[i] = 0.0
                # Principal remains until conversion/exit
                principal_by_year[i] = 0.0
        ...
        return {
            ...
            'equity_value': loan_amount / equity_stake if equity_stake > 0 else 0
        }
    
    elif sub_type == 'royalty':
        royalty_type = loan.get('royaltyType', 'percentage')
        
        if royalty_type == 'percentage':
            royalty_percentage = float(loan.get('royaltyPercentage', 5)) / 100
            # Royalty payments are based on revenue
            # For modeling, we'll estimate based on loan amount
            estimated_annual_revenue = loan_amount * 2  # Assume 2x revenue multiple
            annual_royalty = estimated_annual_revenue * royalty_percentage
        else:  # fixed
            annual_royalty = float(loan.get('fixedRoyaltyAmount', 0))
        ...
        for i in range(forecast_years):
            if i >= base_result['start_idx']:
                interest_by_year[i] = annual_royalty
                principal_by_year[i] = 0.0  # No principal payments for royalty loans
        ...
        return {
            ...
            'royalty_percentage': float(loan.get('royaltyPercentage', 0)) if royalty_type == 'percentage' else None,
            'fixed_royalty_amount': float(loan.get('fixedRoyaltyAmount', 0)) if royalty_type == 'fixed' else None
        }
    
    else:  # fixed
        # Standard fixed payment loan
        return calculate_standard_loan(loan, base_result, forecast_years)

def calculate_standard_loan(loan: Dict[str, Any], base_result: Dict[str, Any], forecast_years: int) -> Dict[str, Any]:
    """
    Calculate standard loan with traditional amortization.
    """
    loan_amount = base_result['loan_amount']
    annual_rate = base_result['annual_rate']
    years_loan = base_result['years_loan']
    
    # Generate standard amortization table
    ...
    for i, row in enumerate(amort_table):
        year_idx = base_result['start_idx'] + i
        if 0 <= year_idx < forecast_years:
            interest_by_year[year_idx] = row['interest']
            principal_by_year[year_idx] = row['principal']
...
def calculate_retail_statements(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if base_loans > 0 and loans:
        for loan in loans:
            # Use new comprehensive loan calculation system
            loan_result = calculate_loan_payments(loan, forecast_years, base_year, interest_rates, data)
            
            # Add to amortization tables
            if loan_result['amortization_table']:
                amortization_tables.append(loan_result['amortization_table'])
            
            # Aggregate interest and principal by year
            for i in range(forecast_years):
                total_interest_by_year[i] += loan_result['interest_by_year'][i]
                total_principal_by_year[i] += loan_result['principal_by_year'][i]
            
            # Calculate balance sheet impacts
            principal_remaining = loan_result['loan_amount']
            for i, row in enumerate(loan_result.get('amortization_table', [])):
                year_idx = loan_result['start_idx'] + i
                if 0 <= year_idx < forecast_years:
                    principal_remaining_by_year[year_idx] += principal_remaining
                    
                    # Short-term debt: principal due next year
                    if i + 1 < len(loan_result['amortization_table']):
                        short_term_debt_by_year[year_idx] += loan_result['amortization_table'][i+1]['principal']
                    else:
                        short_term_debt_by_year[year_idx] += 0
                    
                    # Long-term debt: remaining principal minus next year's principal
                    next_principal = loan_result['amortization_table'][i+1]['principal'] if i + 1 < len(loan_result['amortization_table']) else 0
                    long_term_debt_by_year[year_idx] += max(principal_remaining - next_principal, 0)
    ...
    for i in range(total_years):
        forecast.append({
            'year': years[i],
            'revenue': revenue[i],
            'cogs': income_statement[i].get('cogs', 0),
            'gross_profit': income_statement[i].get('gross_profit', 0),
            'operating_expenses': income_statement[i].get('operating_expenses', 0),
            'ebit': income_statement[i].get('ebit', 0),
            'net_income': income_statement[i]['net_income'],
...
def calculate_service_statements(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # --- Cash Flow Statement ---
    cash_flow = []
    # Initialize opening cash balance for first year
    opening_cash_balance = self_funding + base_loans
    ending_cash = opening_cash_balance - capex[0]  # Subtract first year's CapEx
    ...
    # Initialize working capital for first year
    prev_ar = 0.0
    prev_ap = 0.0
    ...
    for i in range(total_years):
        ...
        # Calculate working capital for current year
        credit_sales = revenue[i] * credit_sales_percent
        ar = (credit_sales * ar_days) / 365 if ar_days > 0 else 0
        cogs_val = cogs[i]
        ap = (cogs_val * ap_days) / 365 if ap_days > 0 else 0
        ...
        # Add loan repayments from amortization tables
        if amortization_tables:
            for amort_table in amortization_tables:
                if i < len(amort_table):
        ...
        # Set opening cash balance for next year
        opening_cash_balance = ending_cash
    ...
    # --- Forecast Table ---
    forecast = []
    min_len = min(total_years, len(years), len(revenue), len(income_statement), len(cash_flow))
