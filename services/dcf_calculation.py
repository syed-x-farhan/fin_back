...
def calculate_scenario_kpis(
    ...
    """
    print(f"[DEBUG] Scenario calculation - Input scenario_values: {scenario_values}")
    print(f"[DEBUG] Scenario calculation - Base discount rate: {base_discount_rate}")
    print(f"[DEBUG] Scenario calculation - Base terminal growth: {base_terminal_growth}")
    
    # For base case (empty scenario_values), use original forecast data
    if not scenario_values:
        print(f"[DEBUG] Base case - Using original forecast data without modifications")
        adjusted_forecast = base_forecast.copy()
        # Handle both field name variations
        free_cash_flows = []
        for year in adjusted_forecast:
            fcf = year.get("free_cash_flow", year.get("freeCashFlow", 0))
            free_cash_flows.append(fcf)
        adjusted_discount_rate = base_discount_rate
        adjusted_terminal_growth = base_terminal_growth
    else:
        print(f"[DEBUG] Scenario case - Applying scenario values: {scenario_values}")
    ...
    print(f"[DEBUG] Scenario calculation - Adjusted free cash flows: {free_cash_flows}")
    print(f"[DEBUG] Scenario calculation - Adjusted discount rate: {adjusted_discount_rate}")
    print(f"[DEBUG] Scenario calculation - Adjusted terminal growth: {adjusted_terminal_growth}")
    ...
    print(f"[DEBUG] Scenario calculation - Terminal value: {terminal_value}")
    
    # Calculate KPIs
    # Use DCF calculation (including terminal value) instead of just NPV
    dcf_value = calculate_dcf_value(free_cash_flows, adjusted_discount_rate, terminal_value)
    print(f"[DEBUG] Scenario calculation - DCF Value (including terminal): {dcf_value}")
    
    # Also calculate NPV without terminal value for comparison
    npv = calculate_npv(free_cash_flows, adjusted_discount_rate)
    print(f"[DEBUG] Scenario calculation - NPV (without terminal): {npv}")
    ...
    try:
        # For payback period, use consistent initial investment across scenarios
        # Use the base case first year FCF as the initial investment for all scenarios
        # This ensures the same investment amount but different cash flow recovery rates
        base_first_year_fcf = base_forecast[0].get("free_cash_flow", base_forecast[0].get("freeCashFlow", 0)) if base_forecast else 0
        
        # Use base case initial investment for all scenarios
        # This allows scenario differences to show in payback period
        initial_investment = base_first_year_fcf
        payback_cash_flows = [-initial_investment] + free_cash_flows
        payback_period = calculate_payback_period(payback_cash_flows)
        print(f"[DEBUG] Scenario payback period calculation - base first year FCF: {base_first_year_fcf}")
        print(f"[DEBUG] Scenario payback period calculation - scenario first year FCF: {free_cash_flows[0] if free_cash_flows else 'None'}")
        print(f"[DEBUG] Scenario payback period calculation - initial investment: {initial_investment}")
        print(f"[DEBUG] Scenario payback period calculation - payback cash flows: {payback_cash_flows}")
        print(f"[DEBUG] Scenario payback period result: {payback_period}")
    except Exception as e:
        print(f"[DEBUG] Scenario payback period calculation failed: {e}")
        payback_period = None
    ...
    year_5_revenue = adjusted_forecast[4].get("revenue", 0) if len(adjusted_forecast) > 4 else 0
    
    print(f"[DEBUG] Adjusted forecast length: {len(adjusted_forecast)}")
    print(f"[DEBUG] Adjusted forecast year 5: {adjusted_forecast[4] if len(adjusted_forecast) > 4 else 'N/A'}")
    print(f"[DEBUG] Year 1 revenue: {year_1_revenue}")
    print(f"[DEBUG] Year 5 revenue: {year_5_revenue}")
    ...
    if adjusted_forecast and year_1_revenue > 0:
        ...
        # If gross_profit is not available, calculate it from revenue and COGS
        if year_1_gross_profit == 0:
            year_1_cogs = adjusted_forecast[0].get("cogs", 0)
            year_1_gross_profit = year_1_revenue - year_1_cogs
        
        # If net_income is not available, calculate it from gross profit and operating expenses
        if year_1_net_income == 0:
            year_1_operating_expenses = adjusted_forecast[0].get("operating_expenses", 0)
            year_1_ebit = year_1_gross_profit - year_1_operating_expenses
            year_1_tax_rate = adjusted_forecast[0].get("tax_rate", 0.25)
            year_1_tax_expense = year_1_ebit * year_1_tax_rate
            year_1_net_income = year_1_ebit - year_1_tax_expense
        
        year_1_gross_margin = (year_1_gross_profit / year_1_revenue) * 100
        year_1_net_margin = (year_1_net_income / year_1_revenue) * 100
        
        print(f"[DEBUG] Margin calculation - Revenue: {year_1_revenue}, COGS: {adjusted_forecast[0].get('cogs', 0)}, Gross Profit: {year_1_gross_profit}, Gross Margin: {year_1_gross_margin}%")
        print(f"[DEBUG] Gross margin calculation details:")
        print(f"  Revenue: {year_1_revenue}")
        print(f"  COGS: {adjusted_forecast[0].get('cogs', 0)}")
        print(f"  Gross Profit: {year_1_gross_profit}")
        print(f"  Calculated Gross Margin: {year_1_gross_margin}%")
        print(f"  Operating Margin Adjustment Applied: {scenario_values.get('operatingMargin', 0)}%")
        print(f"  Net Income: {year_1_net_income}")
        print(f"  Net Margin: {year_1_net_margin}%")
    else:
        print(f"[DEBUG] No margin calculation - Revenue: {year_1_revenue}, Forecast length: {len(adjusted_forecast) if adjusted_forecast else 0}")
    
    result = {
        "npv": dcf_value,  # Use DCF value (including terminal) as NPV for scenarios
        "irr": irr,
        ...
        "npv_without_terminal": npv  # Keep the original NPV for reference
    }
    
    print(f"[DEBUG] Scenario calculation - Final result: {result}")
    return result
...
def calculate_sensitivity_analysis(
    ...
    try:
        ...
        print(f"[DEBUG] Sensitivity analysis - Base case NPV: {base_npv}")
